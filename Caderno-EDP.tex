...
\begin{enumerate}
	...
	\begin{center}
		...
		\draw[domain=-0.99:0.99, samples=70] plot (\x, {-2*\x*exp(1/(-1 + \x*\x))/((pow(\x,4) -2*\x*\x + 1))});
		\end{tikzpicture}
	\end{center}

	Além disso, pode-se usar o Teorema Fundamental do Cálculo para demonstrar que a integral da derivada dará zero.
	
	Então, ao concluir que $\int_U u\ D^\alpha \phi\ dx =0$, não podemos deduzir que $u=0$ pois a integral de $D^\alpha \phi$ dará zero. 
