...
\textit{Demonstração.}\begin{enumerate}[(i)]
	...
	Então, $ D^\alpha u \in W^{k-l-|\alpha|,n}(U) $ para todo $k-l-|\alpha|\geq1 \Rightarrow k-l-1 \geq |\alpha|$. Portanto, $D^\beta u \in W^{1,n}(U)$ para $|\beta| \leq k-l-1$. Agora, com o o domínio é limitado, $D^\beta u \in W^{1,s}(U)$ para todo $s \in [1,n]$. Então, pelo \nameref{t:sobolev-ineq-t2}, para todo $s \in [1,n)$, temos que $D^\beta u \in L^{s^*}(U)$, e que $s^* \in (n, \infty)$.
