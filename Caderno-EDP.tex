...
\textit{Demonstração.} Prova por contraposição. Se a estimativa fosse falsa, existiria, para cada inteiro $k\geq1$ uma função $u_k \in W^{1,p}(U)$ satisfazendo \begin{equation}\label{eq:desig-poincare-1}
...
Logo, as funções $\{v_k\}_{k=1}^\infty$ são limitadas em $W^{1,p}(U)$. \textbf{???} Por quê? Se só as derivadas são limitadas?
...
Agora, \eqref{eq:desig-poincare-2} implica que, \begin{equation}\label{eq:desig-poincare-3}
...
\textbf{???}: Pela relação acima, não podemos deduzir assim? \[\int_U v \phi_{x_i}\ dx = 0 \Rightarrow v=0 \text{ q.t.p.}\] E portanto $||v||_{L^p(U)}=0$?
