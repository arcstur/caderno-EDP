...
\begin{enumerate}
	...
	Como $u$ é lipschitz, temos que $||D_i^{-h}u||_{L^\infty(\rn)}\leq \text{Lip}(u)$ (\textbf{???} o que é isso, é a constante da definição de lipschitz?) E então existe uma função $v_i \in L^\infty(\rn)$ e uma subsequência $h_k\rightarrow 0$ tal que
	\[ D_i^{-h_k}u \rightharpoonup v_i \text{ fracamente em } L^2_{\text{loc}}(\rn)\]
	\textbf{$L^2_{loc}$???} o teorema diz que é para espaços reflexivos, e $L^\infty$ não é, supostamente. $L^2_{loc}$ é reflexivo, mas de onde ele surgiu?
...
\paragraph{Teorema 5 (diferenciabilidade q.t.p.)} Seja $u \in W^{1,p}_{\text{loc}}$(U) para algum $n < p \leq \infty$. Então $u$ é diferenciável q.t.p. em $U$, e seu gradiente é igual ao gradiente fraco q.t.p.
...
\begin{enumerate}
	...
	é válido para qualquer função $C^1$ e, por aproximação, para qualquer $v \in W^{1,p}$ com $n<p<\infty$.
...
% 	\]
% 	\textbf{???} Na aula do Carneiro, era $(2\pi i y)^\alpha$. Parece que o Evans define a transformada de Fourier de uma forma diferente.
...
\paragraph{Definição} denotamos por $H^{-1}(U)$  o espaço dual (o espaço de funcionais lineares e limitados) de $H^1_0(U)$. Escreveremos $\langle \ , \ \rangle$ o mapeamento entre $H^{-1}$ e $H^1_0$.
