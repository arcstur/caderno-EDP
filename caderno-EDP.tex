...
Para isso, sendo $u_m$ suave, \begin{align*}
	...
	&=  \int_{B(0,1)} \eta(y) \int_0^1 \frac{d}{dt} (u_m(x-\e t y))\ dt dy \\
	&=  -\e \int_{B(0,1)} \eta(y) \int_0^1 Du_m(x - \e t y)\cdot y\ dt dy
\end{align*}
...
\begin{enumerate}[(i)]
	...
	B[u,v] = (f,v)
	\] para toda $v \in H^1_0(U)$, em que $(\ {,} \ )$ denota o produto interno em $L^2(U)$. \textbf{???} por que em $L^2$? na equação do problema, multiplicando por $v$ e integrando, o lado direito fica $\int_U fv \ dx$, e não``o'` produto interno do $L^2$.
\end{enumerate}
...
\]
Então, $\beta \| u\| \leq \| Au \|$, o que demonstra as duas propriedades acima. \textbf{??? caramba como?}



Agora, demonstraremos que $R(A)=H$. Suponhamos o contrário. Então, como $R(A)$ é fechado, existe um elemento não-nulo $w \in H$ tal que $w \in R(A)^{\perp}$ (complemento ortogonal, com produto interno nulo), o que contradiz a segunda propriedade, pois então $B \| w \|^2 \leq B[w,w] = (Aw,w)=0$.
